...
async function setVolumeFromUrl(url, name = "CTA") {
  if (!url) {
    throw new Error("后端未返回可用的影像路径 (url 为空)");
  }
  console.info("[viewer] load volume", { url, name });
...
async function uploadCtaAndDisplay() {
  try {
    ...
    const data = await response.json();
    if (!data?.file_path) {
      throw new Error(`上传接口响应缺少 file_path: ${JSON.stringify(data)}`);
    }
...
async function segmentCta() {
  try {
    ...
    const data = await response.json();
    if (!data?.cta_path || !data?.mask_path) {
      throw new Error(`分割接口响应缺少路径字段: ${JSON.stringify(data)}`);
    }
    ...
    console.error("[viewer] segmentCta failed", error);
    result.value = error?.message || String(error);
  }
}
